...
- **Claude Code:** count `mcp__claude_ai_*` entries in the session startup context — each connected claude.ai integration injects at least 2 deferred tool entries; full-featured integrations (e.g. data platforms, project management tools) inject dozens more. Also count MCP servers in harness settings.
